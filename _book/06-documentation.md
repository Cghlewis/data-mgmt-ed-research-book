...
3. Any relevant existing data standards
    - Organizations such as the National Institutes of Health and the National Center for Education Statistics have developed repositories (CDE^[https://www.nlm.nih.gov/oet/ed/cde/tutorial/03-100.html] and CEDS^[https://ceds.ed.gov/]) of standardized question wording paired with a set of allowable response options for commonly used data elements [@national_institutes_of_health_common_nodate; @national_center_for_education_statistics_common_nodate]. Using standards when creating commonly collected variables, such as demographics, provides the following benefits [@icpsr_introduction_2022; @kush_fair_2020]:
      - Reduces bias
...
If your repository provides limited forms for metadata entry, you can also choose to increase the searchability of your files by creating your own machine-readable documents. There are several tools to help users create machine-readable codebooks and data dictionaries that will be findable through search engines such as Google Dataset Search [@arslan_how_2018; @buchanan_getting_2021;@usgs_tools_nodate]. 

### Standards {#metastandards}

Metadata standards, typically field specific, establish a common way to describe your data which improves data interoperability as well as the ability of users to find, understand, and use data [@bolam_guides_nodate]. Metadata standards can be applied in several ways [@bolam_guides_nodate; @ddi_alliance_controlled_nodate]. 
