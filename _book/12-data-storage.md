...
1. Reviewing additional criteria
    - After narrowing down storage solutions based on available tools (e.g., cloud storage, institution network drive, personal device) that meet your compliance needs, electronic data storage locations can be further narrowed based on other criteria.
      ...
      - Size of the storage space: You will need to make sure your storage contains enough space for your files. Consider how many files you will be storing, as well as the size of your files (e.g., number of rows and columns in each dataset)
...
4. Set up additional security systems
    ...
    - User access
      - Assign user access to folders and files based sensitivity levels, quality control needs, and applicable policies, agreements, or plans.
...
1. Choose a final storage location
    - Choose your final location based on available tools and applicable laws, policies, and agreements. Most commonly required, is if paper files contain PII, they should be stored behind two locked doors (e.g., a locked file cabinet in a locked storage room).
...
2. Make a plan for retention and destruction
    ...
    - Electronic
      - For electronic data long-term storage, you will want to consider two things, file formats and storage location [@briney_data_2015].
        - File formats 
      ...
      - If maintaining your electronic data long-term sounds like too much effort for your team, there are other options. Many universities have institutional repositories that often include services such as data curation and preservation. Additionally, there are several external repositories that offer curation and preservation services where you may be able to deposit your data for long-term storage. It's possible that depositing your data in one of these two options may also align with publicly sharing your data, which we will review in Chapter \@ref(share).

3. Consider how you will share data internally
    - Although this may come up prior to the end of the project, it is important to consider where you will store final datasets, how you will notify team members of their availability, and how you will allow team members to access data. An example of this process may look like this:
      - Storing all finalized datasets in a "master dataset folder" (see Chapter \@ref(clean) for more information)
      - Adding descriptions of the finalized datasets to a data inventory (see Section \@ref(document-inventory)) that is shared on a team wiki
      - Creating a data request process for team members to get primary investigator (PI) approval before accessing data files
