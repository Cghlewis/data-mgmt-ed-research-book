...
```{r fig9-1, fig.cap="Tracking in the research project life cycle"}
...
```{r fig9-3, fig.cap="Participant database built in using a non-relational model with duplicated variables denoted by rectangles"}
...
```{r fig9-4, fig.cap="Example participant database model using two separate tables for tracking across waves"}

knitr::include_graphics("img/fig9-4.PNG")
...
```{r fig9-5, fig.cap="Example participant database model using one table to track data across waves"}

knitr::include_graphics("img/fig9-5.PNG")
...
knitr::include_graphics("img/fig9-6.PNG")
...
```{r fig10-7, fig.cap="Example form view data entry"}
...
```{r fig10-8, fig.cap="Example of keeping participant IDs for the entire study", out.width = "70%"}
...
tibble::tribble (~stu_id, ~tch_id, ~sch_id,
  ...
  knitr::kable(caption = "Example of a study id schema created using best practices")
