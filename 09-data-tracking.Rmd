...
```{r fig10-2, fig.cap="Participant database built using a relational model"}
...
```{r fig10-3, fig.cap="Participant database built in using a non-relational model with duplicated variables denoted by rectangles"}
...
```{r fig10-4, fig.cap="Example participant database model using two separate tables for tracking across waves"}
...
```{r fig10-5, fig.cap="Example participant database model using one table to track data across waves"}
...
```{r fig10-6, fig.cap="Example spreadsheet view data entry"}
