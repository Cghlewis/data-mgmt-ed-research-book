...
```{r fig9-2, fig.cap="Timeline for constructing and using a tracking database"}

knitr::include_graphics("img/tracking_db3.PNG")
...
```{r fig9-3, fig.cap="Participant database built using a relational model"}
...
```{r, caption = "Example roster created by querying our relational database tables"}
...
```{r fig9-8, fig.cap="Example form view data entry", out.width= "60%"}
...
```{r fig9-9, fig.cap="Example of a study id schema"}
...
```{r fig9-10, fig.cap="Example of keeping participant IDs for the entire study", out.width= "70%"}
