...
```{r fig11-2, fig.cap="Common data capture methods"}
...
```{r fig11-3, fig.cap="Two different ways you may export a select all question"}
...
```{r fig11-4, fig.cap="The flow of the decisions to make regarding the data entry process"}
...
```{r fig11-5, fig.cap="A report displaying differences between two entry files"}
...
```{r fig11-6, fig.cap="Example non-public confidential data request process"}
...
```{r fig11-7, fig.cap="Example variable request for an external data provider"}
...
```{r fig11-8, fig.cap="Sample documentation form for an external data provider to complete"}
